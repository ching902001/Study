...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        fast_ema = btind.ema.ExponentialMovingAverage(period=self.p.fast)
        slow_ema = btind.ema.ExponentialMovingAverage(period=self.p.slow)
        # long_ema = btind.EMA.ExponentialMovingAverage(period=self.p.long)

        self.fast_signal = btind.CrossUp(fast_ema, slow_ema)
        # self.slow_signal = btind.CrossUp(slow_ema, long_ema)
        self.sell_signal = btind.CrossDown(fast_ema, slow_ema)
    ...
    def next(self):
        ...
        if self.data.close[0] > self.slow_ema[0] & self.fast_signal > 0.0 & self.slow_signal > 0.0:
            self.log(f'BUY {self.getsizing()} shares of {self.data._name} at {self.data.close[0]}')
            self.buy()
            self.orderid = 1
            self.sell_price = (self.data.close[0]-self.slow_ema[0]) * 1.5 + self.data.close[0]

        # Sell when price below long EMA and fast EMA crosses below slow EMA
        elif self.data.close[0] < self.slow_ema[0] & self.sell_signal < 0.0:
            if not self.p.longonly:
                self.log(f'SELL {abs(self.getsizing())} shares '
                         f'of {self.data._name} at {self.data.close[0]}')
                self.sell()
                self.orderid = 2
                self.buy_price = self.data.close[0] - (self.slow_ema[0]-self.data.close[0]) * 1.5
                if self.buy_price < 0:
